...
public abstract class PrePostProcessingCommandInterceptor extends CommandInterceptor {
   ...
   public Object visitRemoveLargeObjectCommand(InvocationContext ctx, RemoveLargeObjectCommand command) throws Throwable {
      try {
         return (doBeforeCall(ctx, command)) ? handleRemoveLargeObjectCommand(ctx, command) : null;
      }
      finally {
         doAfterCall(ctx, command);
      }
   ...
   protected Object handleRemoveLargeObjectCommand(InvocationContext ctx, RemoveLargeObjectCommand command) throws Throwable {
      return handleDefault(ctx, command);
   }
